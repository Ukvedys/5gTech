...
function g5tech_project_image( $post_id, $size = 'large', $decorative = false ) {
	...
	if ( g5tech_is_demo_attachment( $image_id ) ) {
		return sprintf(
			'<img src="%1$s" alt="%2$s" loading="%3$s">',
			esc_url( get_theme_file_uri( 'assets/images/generated/project-telecom-site-v1.jpg' ) ),
			esc_attr( $decorative ? '' : 'Telekomunikacijų bokštas ir techninės infrastruktūros aikštelė' ),
			$decorative ? 'lazy' : 'eager'
		);
	}
